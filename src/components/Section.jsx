...
const Section = () => {
  ...
  useEffect(() => {
    ...
    const selectedS3T = section3Tokens.filter(doc => doc.obj && !doc.obj.lunch).map(doc => doc.tokenNo);
    console.log(selectedS3T);
    setSection3(selectedS3T)

    const section4Tokens=tokenDocuments.slice(15, 20);
    const selectedS4T = section4Tokens.filter(doc => doc.obj && !doc.obj.lunch).map(doc => doc.tokenNo);
    setSection4(selectedS4T)

    const section5Tokens=tokenDocuments.slice(20, 25);
    const selectedS5T = section5Tokens.filter(doc => doc.obj && !doc.obj.lunch).map(doc => doc.tokenNo);
    setSection5(selectedS5T)

    const section6Tokens=tokenDocuments.slice(25, 30);
    const selectedS6T = section6Tokens.filter(doc => doc.obj && !doc.obj.lunch).map(doc => doc.tokenNo);
    setSection6(selectedS6T)

    const section7Tokens=tokenDocuments.slice(30, 35);
    const selectedS7T = section7Tokens.filter(doc => doc.obj && !doc.obj.lunch).map(doc => doc.tokenNo);
    setSection7(selectedS7T)

    const section8Tokens=tokenDocuments.slice(35, 40);
    const selectedS8T = section8Tokens.filter(doc => doc.obj && !doc.obj.lunch).map(doc => doc.tokenNo);
    setSection8(selectedS8T)

    const section9Tokens=tokenDocuments.slice(40, 45);
    const selectedS9T = section9Tokens.filter(doc => doc.obj && !doc.obj.lunch).map(doc => doc.tokenNo);
